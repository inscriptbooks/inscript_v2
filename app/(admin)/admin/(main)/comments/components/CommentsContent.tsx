...
type SortColumn = "createdAt" | null;
...
function CommentsContentComponent({
  initialData,
}: {
  ...
  const { comments, pagination, statistics } = initialData;
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await fetch("/api/admin/comments/export");
      if (!response.ok) {
  ...
  return (
    <div className="flex w-full flex-col gap-8 p-11">
      {/* 제목 */}
      <h1 className="text-2xl font-bold text-gray-1">댓글 관리</h1>
...
export default function CommentsContent({
  ...
  return (
    <Suspense fallback={<div>로딩 중...</div>}>
      <CommentsContentComponent initialData={initialData} />
    </Suspense>
  );
}
