...
export default function BlacklistModal({
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch("/api/admin/members/blacklist", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        showSuccessToast("블랙리스트가 등록되었습니다.");
        onClose();
      } else {
        showErrorToast("블랙리스트 등록에 실패했습니다.");
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative flex w-[520px] flex-col items-center gap-[46px] rounded-xl bg-white p-11 shadow-[0_0_10px_0_rgba(146,46,0,0.08)]">
        ...
        <Button
          ...
          className="flex w-full items-center justify-center gap-2.5 rounded bg-black px-[55px] py-5 hover:bg-black/90"
        >
