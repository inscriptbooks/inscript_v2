...
export default function PromotionDashboard({
  ...
  const promotionItems: Post[] = selectedPage?.posts || [];

  const handlePromotionItemClick = (item: Post) => {
    router.push(`/community/promotion/${item.id}`);
  };
    ...
    <div className="flex w-full flex-col pt-3 lg:pt-7">
      <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
        <div className="flex flex-col gap-3">
          <h2 className="text-xl font-bold text-primary">얘기하기</h2>
          <p className="text-gray-2">얘기하기 게시판</p>
        </div>
        ...
        {promotionItems.length > 0 ? (
          <>
            <div className="grid grid-cols-1 gap-3 self-stretch md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
              {/* Posts grid */}
              {promotionItems.map((post) => (
                <Card
                  key={post.id}
                  className="h-full w-full cursor-pointer border border-red-3"
                  onClick={() => handlePromotionItemClick(post)}
                >
            ...
            <div className="mt-6 flex w-full flex-col items-center gap-6 lg:flex-row lg:items-center lg:justify-between">
              ...
              />
              <CommunityPostButton type="promotion" />
            </div>
          </>
        ) : (
